...
import { TOOL_NAME_COMPATIBILITY_CHECK } from "@/lib/constants";
...
export async function POST(request: NextRequest) {
  const auth = await authenticateAndCheckUsage(TOOL_NAME_COMPATIBILITY_CHECK);
  ...
  await incrementUsage(auth.context.userId, TOOL_NAME_COMPATIBILITY_CHECK);
