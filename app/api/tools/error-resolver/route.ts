...
import { TOOL_NAME_ERROR_RESOLVER } from "@/lib/constants";
...
export async function POST(request: NextRequest) {
  const auth = await authenticateAndCheckUsage(TOOL_NAME_ERROR_RESOLVER);
  ...
  await incrementUsage(auth.context.userId, TOOL_NAME_ERROR_RESOLVER);
