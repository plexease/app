...
import { TOOL_NAME_TOOL_FINDER } from "@/lib/constants";
...
export async function POST(request: NextRequest) {
  const auth = await authenticateAndCheckUsage(TOOL_NAME_TOOL_FINDER);
  ...
  await incrementUsage(auth.context.userId, TOOL_NAME_TOOL_FINDER);
