...
import { UpgradeAssistantPage } from "../../pages/upgrade-assistant.page";

test.describe("Upgrade Assistant", () => {
  test("free user submits migration and sees result cards", async ({ freeUserPage, mockApi }) => {
    await mockApi.upgradeAssistant(freeUserPage, "success");

    const assistant = new UpgradeAssistantPage(freeUserPage);
  ...
  test("shows error message on API failure", async ({ freeUserPage, mockApi }) => {
    await mockApi.upgradeAssistant(freeUserPage, "error");
  ...
  test("pro user sees usage counter", async ({ proUserPage, mockApi }) => {
    await mockApi.upgradeAssistant(proUserPage, "success");

    const assistant = new UpgradeAssistantPage(proUserPage);
    await assistant.goto();

    await expect(assistant.usageCounter).toBeVisible();
  });
});
